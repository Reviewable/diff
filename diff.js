...
diff.prototype.diff_mainInternal_ = function(text1, text2, checklines,
  ...
  return results.length ? results.pop() : [];
};
...
diff.prototype.diff_computeOperation_ = function(text1, text2, checklines,
  ...
  if (i != -1) {
    ...
    diffs = [new diff.Diff(DIFF_INSERT, longtext.substring(0, i)),
             new diff.Diff(DIFF_EQUAL, shorttext),
             new diff.Diff(DIFF_INSERT,
    ...
    if (diffs[pointer - 1][0] == DIFF_DELETE &&
      ...
      if (overlap_length1 >= overlap_length2) {
        if (overlap_length1 >= deletion.length / 2 ||
          ...
          diffs.splice(pointer, 0, new diff.Diff(DIFF_EQUAL,
...
diff.prototype.diff_cleanupMerge = function(diffs) {
  var changes;
  // Re-run the merge passes iteratively when a shift exposes new work.
  do {
    changes = false;
    // Add a dummy entry at the end.
    diffs.push(new diff.Diff(DIFF_EQUAL, ''));
          ...
          if (count_delete + count_insert > 1) {
            if (count_delete !== 0 && count_insert !== 0) {
              ...
              if (commonlength !== 0) {
                if ((pointer - count_delete - count_insert) > 0 &&
                  ...
                  diffs.splice(0, 0, new diff.Diff(DIFF_EQUAL,
                      text_insert.substring(0, commonlength)));
                  pointer++;
                }
            ...
            if (text_delete.length) {
              diffs.splice(pointer, 0,
                  new diff.Diff(DIFF_DELETE, text_delete));
              pointer++;
            }
            if (text_insert.length) {
              diffs.splice(pointer, 0,
                  new diff.Diff(DIFF_INSERT, text_insert));
              pointer++;
            }
            pointer++;
